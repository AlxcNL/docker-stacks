...
# the student user, and ensure file permissions, grant sudo rights, and such
...
if [ "$(id -u)" == 0 ] ; then
    ...
    # Refit the student user to the desired the user (NB_USER)
    if id student &> /dev/null ; then
        if ! usermod --home "/home/${NB_USER}" --login "${NB_USER}" student 2>&1 | grep "no changes" > /dev/null; then
            _log "Updated the student user:"
            _log "- username: student       -> ${NB_USER}"
            _log "- home dir: /home/student -> /home/${NB_USER}"
        fi
    elif ! id -u "${NB_USER}" &> /dev/null; then
        _log "ERROR: Neither the student user or '${NB_USER}' exists. This could be the result of stopping and starting, the container with a different NB_USER environment variable."
        exit 1
    fi
    ...
    # Move or symlink the student home directory to the desired users home
    # directory if it doesn't already exist, and update the current working
    # directory to the new location if needed.
    if [[ "${NB_USER}" != "student" ]]; then
        if [[ ! -e "/home/${NB_USER}" ]]; then
            _log "Attempting to copy /home/student to /home/${NB_USER}..."
            mkdir "/home/${NB_USER}"
            if cp -a /home/student/. "/home/${NB_USER}/"; then
                _log "Success!"
            else
                _log "Failed to copy data from /home/student to /home/${NB_USER}!"
                _log "Attempting to symlink /home/student to /home/${NB_USER}..."
                if ln -s /home/student "/home/${NB_USER}"; then
                    _log "Success creating symlink!"
                else
                    _log "ERROR: Failed copy data from /home/student to /home/${NB_USER} or to create symlink!"
                    exit 1
                fi
            fi
        fi
...
# user we started as.
    ...
    student_UID="$(id -u student 2>/dev/null)"  # The default UID for the student user
    student_GID="$(id -g student 2>/dev/null)"  # The default GID for the student user
    ...
    if ! whoami &> /dev/null; then
        ...
        if [[ -w /etc/passwd ]]; then
            _log "Renaming old student user to nayvoj ($(id -u student):$(id -g student))"
            ...
            sed --expression="s/^student:/nayvoj:/" /etc/passwd > /tmp/passwd
            echo "${NB_USER}:x:$(id -u):$(id -g):,,,:/home/student:/bin/bash" >> /tmp/passwd
            ...
            if [[ "${NB_USER}" != "student" ]]; then
                _log "WARNING: user is ${NB_USER} but home is /home/student. You must run as root to rename the home directory!"
            fi
        else
    ...
    if [[ ! -w /home/student ]]; then
        _log "WARNING: no write access to /home/student. Try starting the container with group 'users' (100), e.g. using \"--group-add=users\"."
    fi
